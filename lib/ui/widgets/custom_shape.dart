...
class CustomShapeClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.lineTo(0.0, size.height - 820);

    var firstEndPoint = Offset(size.width * .5, size.height - 630.0);
    var firstControlpoint = Offset(size.width * 0.25, size.height - 450.0);
    path.quadraticBezierTo(firstControlpoint.dx, firstControlpoint.dy,
        firstEndPoint.dx, firstEndPoint.dy);

    var secondEndPoint = Offset(size.width, size.height - 450.0);
  ...
  bool shouldReclip(CustomClipper oldClipper) =>
      true; //eta en true para que se actualice cada que haya un cambio
}
